...
; 06-Jun-2022, Kim. In init method, when setting structures from the .plotman...geny preference files,
;   remove the tag dummy if it's there.  Breaks everything because it goes into plot_args which should
;   have {dummy:0} if nothing else is put into it (and in user struct, could have different value)
;-
;
;=========================================================================
...
function plotman::init, input=input, plot_type=plot_type, $
  ...
  if file_test(self.xy_file_pref,/read) then begin
    ...
    if is_struct(struct) then struct=rem_tag(struct,'dummy')
    *self.xy_pref = struct
  endif
